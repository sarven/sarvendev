...
leads to Remote Code Execution (RCE) vulnerability. The probability of such an attack is quite low, because the attacker would need to know about this specific parameter and the fact that it is deserialized, 
...
Keep in mind potential memory problems if you store too much data there, but if it's just a few small objects, it should be fine. Thinking about this kind of memory issues is especially important
